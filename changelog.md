...
- Admin item import panel (visible only to the configured admin email).
  ...
  documents or merging into existing ones (duplicates matched by name are
  skipped).
- `js/admin.js` — `initAdmin()`, file parse/preview, and
  `importSourceBook()` merge logic.
...
- `showView()` now syncs the active nav-link highlight when switching
  views.
- Nav links in `#main-nav` now correctly call `showView()` on click.
- Admin nav link click was not wired to `showView('admin')`.
